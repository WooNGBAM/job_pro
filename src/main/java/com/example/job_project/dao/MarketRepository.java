...
import com.example.job_project.VO.BusinessVO;
...
public interface MarketRepository extends CrudRepository<Market, Integer> {
    ...
    @Query(value = "select open from market_business_times where market_id = ?1 AND day LIKE ?2%", nativeQuery = true)//개점시각
    LocalTime findByIdOpen(int id, String today);//인자로 오늘날짜(요일로) 넣어주기*/

    @Query(value = "select close from market_business_times where market_id = ?1 AND day LIKE ?2%", nativeQuery = true)//폐점시각
    LocalTime findByIdClose(int id, String today);//인자로 오늘날짜(요일로) 넣어주기

    @Query(value = "select day, open, close, business_status from market_business_times where market_id = ?1 AND day LIKE ?2%", nativeQuery = true)
    Business findbusi(int id, String today);
